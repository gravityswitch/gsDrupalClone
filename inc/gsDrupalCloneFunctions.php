...
function setupAliasFile($CLONE_CONF, $SITE_CONF) {
	...
	shell_exec("ssh $remoteUser@$remoteSite \"drush --root=\\\"$remoteDir\\\" site-alias --with-db --show-passwords --with-optional @self\" > $aliasFile.tmp");

	shell_exec("echo '<?php' > .$localSite.alias.tmp && cat $aliasFile.tmp >> .$localSite.alias.tmp");
	...
	$aliases["$localSite.$localDevTLD"]['root'] = $baseDir . $localSite . "/$wwwroot";
	$aliases["$localSite.$localDevTLD"]['path-aliases']['%drush'] = $drush;
	
	// Write the array back out to the alias file in PHP format
	$results = "<?php \$aliases[\"$remoteSite\"] = " . var_export($aliases["$remoteSite"], true) . ";\n";

	$results .= "\$aliases[\"$localSite.$localDevTLD\"] = " . var_export($aliases["$localSite.$localDevTLD"], true) . ";\n";
...
function syncFiles($CLONE_CONF, $SITE_CONF) {
	...
	shell_exec("drush -y rsync --include-conf @$remoteSite @$localSite.$localDevTLD");
...
function setupDB($CLONE_CONF, $dbLocalName, $dbLocalUser, $dbLocalPassword) {
	...
	$sql = "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER, CREATE TEMPORARY TABLES ON $dbLocalName.* TO \"$dbLocalUser\"@'localhost' IDENTIFIED BY \"$dbLocalPassword\"";
	if ($localDB->query($sql) === TRUE) {
...
function populateDB($CLONE_CONF, $SITE_CONF, $dbLocalName) {
	...
	$sql = preg_replace("/$remoteSiteSearchStr/", "$localSite.$localDevTLD", $remoteDump);

	// Write the contents back to the file
	file_put_contents($file, $sql);

	// Populate the local DB with the modified DB Dump
	shell_exec("mysql -u $dbRootUsername -p$dbRootPassword $dbLocalName < sqldump.sql");
